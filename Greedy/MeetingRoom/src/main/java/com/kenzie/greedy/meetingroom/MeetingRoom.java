...
public class MeetingRoom {
    ...
    public static int getMaximumMeetings(List<Meeting> meetings) {
        ...
        ArrayList<Meeting> meetingResult = new ArrayList<>();
        int schdeuleEndTime = 0;

        meetings.sort(Comparator.comparingInt(Meeting::getEndTime));

        for (Meeting meeting : meetings) {
            if (meeting.getStartTime() >= schdeuleEndTime) {
                meetingResult.add(meeting);
                schdeuleEndTime = meeting.getEndTime();
            }
        }
